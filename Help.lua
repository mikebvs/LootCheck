...
Help.COMMANDS = {
    ...
    { cmd = "/lchelp graph", args = "[days]", aliases = { "/lchg" },
      desc = "Open the wishlist award graph. The bars count awards that match the wishlist you currently have imported. The < > buttons filter by content phase instead (see /lchelp phase), and a number limits the bars to awards from the last that many days (0 = no limit). Both are remembered. Beside the bars is the week's raid drops, and the Character button pops a raider's sheet out on the right (see /lchelp sheet)." },
    ...
    { cmd = "/lchelp sheet", args = "[character]", aliases = { "/lchs", "/lchelp character", "/lchelp char" },
      desc = "Open the wishlist graph with the character popout out: pick a raider and see what they want in each equipment slot and the rank they gave it, beside their bar and the week's drops. The Character button on the graph opens and closes it, and the window widens to make room rather than squeezing the other columns. Slots with nothing on their wishlist are shown empty, so a gap is as visible as a want, and received items are greyed exactly as on the tooltip. Tier tokens are placed by name, since the client has no slot for an item that is not equippable; anything else the client has not cached yet waits under \"Slot not known yet\" and moves up once the data arrives. Clicking a raider's bar pops their sheet out on them." },
    ...
    { cmd = "/lchelp drops", args = "[clear]",
      desc = "Open the wishlist graph, whose right-hand column lists every rare and epic item that dropped in the raid this week, newest first, with who picked it up and who Gargul assigned it to. Those are separate columns on purpose: an item is often looted by whoever had bag space and only assigned later. Drops are read from the loot window and from loot messages in chat, which every client in the raid sees, so no addon-to-addon communication is needed. The arrows step back through earlier weeks; the list rolls over at the Tuesday raid reset. \"Gear only\" hides everything that is not worn or wielded, so gems, reagents and recipes drop out and tier tokens stay. Hovering a drop shows its tooltip, so the wishlist is right there. \"clear\" forgets every recorded drop." },
