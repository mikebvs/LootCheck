...
Companion addon for **Gargul** + **That's My BIS Export (TMBExport)** on WoW TBC Anniversary.
...
`python tests\harness.py` runs the addon inside a real Lua 5.1 VM (OBS Studio's `lua51.dll`) with a small WoW-API stub, against the real saved variables, and checks tooltip output, greying, settings handling and the graph counts against an independent re-implementation.
