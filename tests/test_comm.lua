...
LootCheck.Imports:Add("Send Test", csv({
    ...
    "John,Shaman,2,Band of Devastation,30347,0",
    "Jill,Shaman,1,Tsunami Talisman,30627,0",
}))
...
FakeComm:Reset()
FakeComm:Deliver("LootCheck", "P|1", "RAID", "John")
local reply = FakeComm:Last("R|")
assert(reply and reply.distribution == "WHISPER" and reply.target == "John", "reply whispered back")
assert(reply.text:find(LootCheck.version, 1, true), "reply carries our version")
-- Their reply marks them as having the addon
assert(not LootCheck.Comm:HasAddon("john"), "not known yet")
FakeComm:Deliver("LootCheck", "R|1|1.0.0", "WHISPER", "John")
assert(LootCheck.Comm:HasAddon("john"), "peer recorded")

section("send: only to the players who were ticked")
FakeComm:Reset()
local out = capture(function()
    LootCheck.Comm:SendDataset({ "John", "Jill" })
end)
...
assert(seen.John and seen.Jill and not seen["Mary-Testrealm"], "only the chosen players")
...
FakeComm:Deliver("LootCheck", payload, "WHISPER", "John")
...
assert(LootCheckCommPrompt.text:GetText():find("John", 1, true), "prompt names the sender")
...
assert(out:find("imported 'Send Test (from John)'", 1, true), out)
assert(#LootCheck.Imports:List() == before + 1, "saved as a new raid")
assert(not LootCheckCommPrompt:IsShown(), "prompt closed")
assert(LootCheck.Imports:Active().name == "Send Test (from John)", "the new raid became active")
LootCheck.Imports:SetActive(mine.id, true)

section("receiving: Merge keeps local rows and adds only what is new")
local incoming = csv({
    "John,Shaman,7,Tsunami Talisman,30627,0", -- duplicate, different prio
    "John,Shaman,1,Tsunami Talisman,30627,1", -- same item, off-spec: not a duplicate
    "Mary,Warlock,1,Leggings of the Vanquished Hero,30247,0", -- new
})
local encoded = "D|1|" .. LootCheck.Comm:Encode(incoming)
FakeComm:Deliver("LootCheck", encoded, "WHISPER", "Mary-Testrealm")
...
assert(mine.mergedFrom == "Mary", "merge is recorded on the import")
local index = LootCheck.Data:WishlistIndex()
assert(index[30627]["john"] and #index[30627]["john"].entries == 2, "MS kept plus the new OS entry")
assert(index[30627]["john"].bestPrio == 1, "the local priority survived the merge")
assert(index[30247]["mary"], "the new character came through")
...
FakeComm:Deliver("LootCheck", encoded, "WHISPER", "John")
FakeComm:Deliver("LootCheck", encoded, "WHISPER", "Jill")
...
FakeComm:Deliver("LootCheck", "D|9|" .. LootCheck.Comm:Encode(incoming), "WHISPER", "John")
assert(#LootCheck.Comm.queue == 0, "another protocol version is ignored")
out = capture(function() FakeComm:Deliver("LootCheck", "D|1|not-actually-compressed", "WHISPER", "John") end)
...
-- The list is alphabetical, so find the peer rather than assume a position
local peerRow
for _, row in ipairs(shown) do
    if LootCheck:NormalizeName(row.playerName) == "john" then peerRow = row end
end
assert(peerRow and peerRow.label:GetText():find("LootCheck", 1, true),
    "the player who replied to the ping is marked as having the addon")
